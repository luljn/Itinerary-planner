/*
 * Class Carte :
 * D�riv�e de ImageScrollable (pour l'image)
 * Contient et dessine l'itin�raire
 */
...
public class Map extends ImageScrollable {
    ...
    // Donn�es du point le plus proche
	...
	private void init(){
		...
		// Option "chosir comme point de d�part"
		...
		// Option "Changer la couleur du trac�"
    ...
    public void afficherMenu() {
    	if ((point_proche.getX() != -1) && (point_proche.getY() != -1)) {
	    	...
	    	// Annulation du d�placement de la carte
    ...
    synchronized public void paintComponent(Graphics g) {
    	...
    	if (itineraire.size() > 1) {
	    	...
	    	for (Iterator it = itineraire.iterator(); it.hasNext();) {
	    		...
	    		if (prec.getX() == -1) {
	    			...
	    			// Tra�age de la droite eliant les 2 points
	    			g2.drawLine((int)prec.getX(), (int)prec.getY(), (int)pt.getX(), (int)pt.getY());
	    		}
	    		prec.setLocation(pt);
	    	}
	    	
	    	// Premier point : tra�age du point de d�part
	    	tracePointDepart(g2, pt_depart);
	    	
	    	// Dernier point : tra�age du point d'arriv�e
    	...
    	// Tra�age du point le plus proche de la souris
    	if ((point_proche.getX() != -1) && (point_proche.getY() != -1)) {
